...
function toDebugResult(ranked: RankedProperty): DebugResult {
  const { property, score, breakdown, explanation, rank, lowConfidence, hardFilterExempted } =
    ranked;
...
function computeDiagnostics(payload: RankingPayload): DebugDiagnostics {
  ...
  const minGap =
    scores.length > 1 ? r3(Math.min(...scores.slice(1).map((s, i) => scores[i] - s))) : 0;
  ...
  const tooAggressive =
    payload.rankingExplanation.hardFilterTriggered &&
  ...
  // ── Coverage ──────────────────────────────────────────────────────────────
  const lowScoreCeiling =
    payload.topScore < DIAG_LOW_SCORE_CEILING && !thinPool && !relaxedModeUsed;
