  ...
  if (s.expectTopResultDimensions) {
    ...
    for (const r of topSlice) {
      for (const [dim, exp] of Object.entries(dims)) {
        ...
        if (exp.min !== undefined && val < exp.min) {
          failures.push(
            `#${r.rank} ${r.property.name.replace("Zostel ", "")} ${dim}=${val.toFixed(2)} < min ${exp.min}`
          );
        }
        if (exp.max !== undefined && val > exp.max) {
          failures.push(
            `#${r.rank} ${r.property.name.replace("Zostel ", "")} ${dim}=${val.toFixed(2)} > max ${exp.max}`
          );
        }
      }
    }
    ...
    if (failures.length === 0) {
      const dimStr = Object.entries(dims)
        .map(
          ([d, e]) =>
            `${d}${e.min !== undefined ? `≥${e.min}` : ""}${e.max !== undefined ? `≤${e.max}` : ""}`
        )
        .join(", ");
...
function printScenario(
  ...
  // ── Pipeline summary ─────────────────────────────────────────────────────
  const confColor =
    payload.confidence === "high" ? GRN : payload.confidence === "moderate" ? YLW : RED;
  const filterLine =
    payload.hardFilteredCount > 0
      ? `  ${RED}hardFiltered=${payload.hardFilteredCount}${R}${payload.rankingExplanation.relaxedModeUsed ? `  ${YLW}(relaxed)${R}` : ""}`
      : "";
  const fallbackLine = payload.fallback ? `  ${YLW}fallback=${payload.fallback}${R}` : "";
  console.log(
  ...
  if (display.length === 0) {
    ...
    if (payload.rankingExplanation.hardFilterTriggered) {
      console.log(
        `  ${YLW}Hard filter triggered${R}  workation threshold=${WORKATION_USER_THRESHOLD}  relaxed=${WORKATION_PROP_RELAXED}`
      );
    }
  } else {
    console.log();
    for (const r of display) {
      ...
      console.log(
        `  ${BOLD}#${r.rank}${R}  ${BOLD}${p.name}${R}  ${DIM}${p.location}${R}  ${flags}`
      );
      console.log(`      score   ${GRN}${scoreBar}${R}  ${BOLD}${r.score.toFixed(3)}${R}`);

      const topM = r.explanation.topMatches
        .slice(0, 3)
        .map((m) => `${GRN}${m.dim}${R}+${m.contribution.toFixed(2)}`)
        .join("  ");
      const topX = r.explanation.topMisses
        .slice(0, 2)
        .map((m) => `${RED}${m.dim}${R}Δ${m.gap.toFixed(2)}`)
        .join("  ");
  ...
  if (diag) {
    ...
    const causeStr = diag.causes.map((c) => `${causeColors[c] ?? R}${c}${R}`).join("  ");
    ...
    if (diag.metadata.suspects.length > 0) {
      const topSuspects = diag.metadata.suspects
        ...
        .map(
          (s) =>
            `#${s.resultRank} ${s.propertyName.replace("Zostel ", "")} ${s.dim}Δ${s.gap.toFixed(2)}`
        )
        .join(", ");
...
function printSummary(rows: SummaryRow[]): void {
  ...
  console.log(
    `  ${DIM}${"─".repeat(catPad)}  ${"─".repeat(idPad)}  ${"─".repeat(namePad)}  ──────  ───────  ──────${R}`
  );
