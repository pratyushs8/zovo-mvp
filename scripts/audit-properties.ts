  ...
  beach_social: [
    {
      ...
      min: 0.6,
      severity: "high",
      rationale: "beach hostels derive their value from social atmosphere",
    },
  ...
  mountain_adventure_hub: [
    {
      ...
      min: 0.6,
      severity: "high",
      rationale: "the archetype's primary value proposition",
    },
  ...
  remote_mountain_quiet: [
    {
      ...
      min: 0.7,
      severity: "high",
      rationale: "remoteness and quiet are the defining traits",
    },
    {
      dim: "scenic",
      min: 0.7,
      severity: "medium",
      rationale: "remote mountain locations are scenic by nature",
    },
    {
      dim: "social",
      max: 0.5,
      severity: "medium",
      rationale: "remote quiet properties shouldn't feel social",
    },
  ],
  cultural_hill_town: [
    { dim: "scenic", min: 0.6, severity: "medium", rationale: "hill towns are generally scenic" },
    {
      ...
      min: 0.5,
      severity: "low",
      rationale: "cultural towns are calmer than beach/city",
    },
  ],
  heritage_cultural_city: [
    {
      ...
      min: 0.4,
      severity: "low",
      rationale: "heritage cities typically offer budget-friendly options",
    },
  ],
  urban_metro: [
    {
      ...
      min: 0.5,
      severity: "high",
      rationale: "urban metros should have reliable work infrastructure",
    },
    {
      dim: "scenic",
      max: 0.5,
      severity: "medium",
      rationale: "metro properties are not primarily scenic",
    },
  ],
  nature_retreat: [
    {
      ...
      min: 0.7,
      severity: "high",
      rationale: "nature settings define the archetype",
    },
...
function auditProperty(p: PropertySeed): AuditIssue[] {
  ...
  if (halfDims.length >= 3) {
    issues.push({
      ...
      suggestion:
        `Research actual property conditions and replace 0.5 placeholders with real values. ` +
  ...
  if (spread < 0.4) {
    issues.push({
      ...
      suggestion:
        `A well-tagged property should have at least one dimension ≥ 0.7 and at least one ≤ 0.3. ` +
...
function main(): void {
  ...
  if (clean.length > 0 && !quietMode) {
    console.log(
      `${GRN}✓  ${clean.length} propert${clean.length === 1 ? "y" : "ies"} with no issues${R}`
    );
    console.log();
  }
  ...
  if (exitCode === 0 && highCount + mediumCount === 0) {
    ...
    console.log(
      `${RED}  ${highCount} high-severity issue${highCount !== 1 ? "s" : ""} require attention before the next ranking run.${R}`
    );
    console.log();
  }
